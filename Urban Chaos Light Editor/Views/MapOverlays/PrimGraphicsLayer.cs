...
{
    /// <summary>
    /// Draws sprite graphics for each prim/object on the map (read-only display).
    /// Images are loaded from embedded resources under Assets/Images/PrimGraphics.
    /// </summary>
    public sealed class PrimGraphicsLayer : FrameworkElement
    {
        ...
        private void RefreshPrims()
        {
            Debug.WriteLine("[PrimGraphicsLayer] Refreshing prims list...");
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            Debug.WriteLine($"[PrimGraphicsLayer] OnRender called, {_prims.Count} prims, Visibility={Visibility}");
            Debug.WriteLine($"[PrimGraphicsLayer] ActualSize={ActualWidth}x{ActualHeight}, Visibility={Visibility}, IsVisible={IsVisible}");
            ...
            foreach (var p in _prims.OrderBy(p => p.Y))
            {
                ...
                if (sprite != null)
                {
                    if (logCount < 5)
                    {
                        Debug.WriteLine($"[PrimGraphicsLayer] Drawing prim {p.PrimNumber} at ({p.PixelX}, {p.PixelZ})");
                        logCount++;
                    }
                    DrawPrimSprite(dc, p.PixelX, p.PixelZ, p.PrimNumber, p.Yaw);
                    drawn++;
                }
                else
                {
            ...
            Debug.WriteLine($"[PrimGraphicsLayer] Rendered {drawn} prims");
            if (missingPrimNumbers.Count > 0)
            {
                Debug.WriteLine($"[PrimGraphicsLayer] Missing sprites for prim numbers: {string.Join(", ", missingPrimNumbers.OrderBy(x => x))}");
            }
        }
        ...
        private PrimSprite? GetPrimSprite(byte primNumber)
        {
            ...
            if (sprite == null)
                Debug.WriteLine($"[PrimGraphicsLayer] MISSING sprite for prim {primNumber}");
        ...
        private static PrimSprite? LoadPrimGraphic(byte primNumber)
        {
            ...
            Debug.WriteLine($"[PrimGraphicsLayer] Trying to load: {uri}");
            ...
            try
            {
                ...
                Debug.WriteLine($"[PrimGraphicsLayer] Loaded prim {primNumber}: {w}x{h}");
