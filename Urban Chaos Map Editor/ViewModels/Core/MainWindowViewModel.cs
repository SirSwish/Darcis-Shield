            ...
            MapDataService.Instance.MapLoaded += (_, e) =>
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    try
                    {
                        ...
                        var objAcc = new PrimsAccessor(svc);
                        ...
                        var view = System.Windows.Application.Current.MainWindow as Views.Core.MainWindow;
                        var mapView = view?.MapViewControl;
                        var bLayer = mapView?.FindName("BuildingLayer") as BuildingLayer;
...
_ => MapDataService.Instance.IsLoaded && Map.SelectedPrim != null);
        ...
        private void ShowPrimProperties()
        {
            ...
            var flagsStr = PrimFlags.FromByte(newFlags).ToString();
            var insideLabel = newInside == 0 ? "Outside" : $"Inside={newInside}";
            StatusMessage = $"Updated {p.Name} | Height={newY} | Flags: [{flagsStr}] | {insideLabel}";
        }
        #endregion
    }
}